...
class docregister_doc(models.Model):
    ...
    refcode = fields.Char('Reference Code', size=32, tracking=True,copy=False)
    direction = fields.Selection([('in', 'Incoming'), ('out', 'Outgoing')],'Direction')
    subject = fields.Char('Subject', size=128, required=True, help="Short summary of the subject of the document", tracking=True)
    externalref = fields.Char('External Code', size=128, help="Document reference code used by partner", tracking=True)
    partner_id = fields.Many2one('res.partner', string='Primary partner', tracking=True)
    partner_ids = fields.Many2many('res.partner', string='Other partner(s)', tracking=True)
    tag_ids = fields.Many2many('docregister.tag', string='Tag(s)', tracking=True)
    archivalcateg_id = fields.Many2one('docregister.archivalcateg', 'Archival Category',  domain=[('heading', '=', False)],tracking=True)
    date_done = fields.Date('Done Date',  help="Date when the document was created and signed", tracking=True)
    date = fields.Date('Date',   required=True, help="Date of arrival (for incoming) or date of sending (for outgoing)", tracking=True)
    type_id = fields.Many2one('docregister.type', 'Type',  tracking=True)
    registered = fields.Selection([('no', 'No'),('registered', 'Registered'),('return', 'Return Receipt')],'Registered Post', default='no')
    date_delivery = fields.Date('Delivery Date', help="For letter with Return Receipt: date when letter was delivered", tracking=True)
    attachmentno = fields.Integer('No. of Attachments', help="Number of documents attached to the main document", tracking=True)
    predoc_id = fields.Many2one('docregister.doc', 'Preceded by', tracking=True)
    postdoc_ids = fields.One2many('docregister.doc',  'predoc_id', 'Followed by',readonly=True, tracking=True)
    basedoc_id = fields.Reference([('calendar.event','Meeting')],'Base Document')
    user_id = fields.Many2one('res.users', string='Assigned to',  tracking=True)
    deadline = fields.Date('Deadline',  help="Deadline for dealing with document", tracking=True)
    folder_id = fields.Many2one('docregister.folder', 'Folder',  tracking=True)
    folderplace = fields.Char('Within folder', size=64, help="Placement of document within the folder", tracking=True)
    note = fields.Text('Note', help='Notes related to the document', tracking=True)
    protected= fields.Boolean('Protected data', help='Document contains protected data', tracking=True, default=False)
    state = fields.Selection([('draft', 'Draft'),('open', 'To be answered'),('fail', 'Postage failed'),('close', 'Closed')],'State', default='draft', tracking=True)
