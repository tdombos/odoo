...
class crm_phonecall(osv.osv):
    ...
    def schedule_another_phonecall(self, cr, uid, ids, schedule_time, call_summary, \
        ...
        if not categ_id:
            try:
                res_id = model_data._get_id(cr, uid, 'phonecall', 'categ_phone2')
    ...
    def redirect_phonecall_view(self, cr, uid, phonecall_id, context=None):
        model_data = self.pool.get('ir.model.data')
        # Select the view
        tree_view = model_data.get_object_reference(cr, uid, 'phonecall', 'crm_case_phone_tree_view')
        form_view = model_data.get_object_reference(cr, uid, 'phonecall', 'crm_case_phone_form_view')
        search_view = model_data.get_object_reference(cr, uid, 'phonecall', 'view_crm_case_phonecalls_filter')
        value = {
                'name': _('Phone Call'),
                'view_type': 'form',
                'view_mode': 'tree,form',
                'res_model': 'phonecall.phonecall',
